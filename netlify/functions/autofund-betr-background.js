...
const contractAddress = '0xaD4Dc4712523B0180da5139Ad11C3FDDc6d7Cf06';
...
async function getAllowances(fid, currencies, percent, tipTime) {
  ...
  for (const coin of currencies) {
    ...
    if (coin == '$TIPN' && tipTime == '7pm') {
      allowance = await getBetrAllowance(fid);
      console.log('$BETR', allowance)
...
async function getBetrAllowance(fid) {
  try {
