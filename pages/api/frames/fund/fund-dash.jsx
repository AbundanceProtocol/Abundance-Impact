  ...
  try {
    ...
    async function getCuratorData(fid) {
      try {
        ...
        return {userData: null, schedule: null}
      }
    }



    const {userData, schedule} = await getCuratorData(Number(fid))
    ...
    const svg = await satori(
      <div style={{
        ...
        <div style={{display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '0.0rem', border: '0px solid #eeeeeeaa', width: 'auto', margin: '5px 5px 0px 5px'}}>
          ...
          <div style={{display: 'flex', textAlign: 'center', color: '#eff', fontSize: '28px', margin: '0', padding: '0px 0 10px 0', fontWeight: '400'}}>Support Farcaster's growth</div>
        ...
        {(schedule?.active_cron && userData) && (<div style={{display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: '0.25rem', border: '0px solid #eeeeeeaa', width: 'auto', margin: '10px 5px 0px 5px'}}>
          ...
          <div style={{display: 'flex', flexDirection: 'row', color: 'black', alignItems: 'center', justifyContent: 'center', gap: '0.25rem', border: '1px solid #eeeeeeaa', borderRadius: '88px', padding: '3px 10px 3px 3px', background: '#eeeeeeaa', width: 'auto', margin: '0 5px 0 0'}}>
            {userData && userData?.pfp && userData?.pfp !== null && (<img src={userData?.pfp} width={30} height={30} style={{borderRadius: '80px', border: '2px solid #eee', backgroundColor: '#8363ca'}} />)}
        ...
        <div style={{display: 'flex', flexDirection: 'row', fontSize: '13px', justifyContent: "center", alignItems: 'center', gap: '0.75rem', margin: '20px 0', flexWrap: 'wrap', width: '100%'}}>
          <div className={`btn-select ${schedule?.active_cron && schedule?.creator_fund == 0 ? 'cast-act-lt btn-brd-lt' : 'blu-drk btn-brd'}`} style={{display: 'flex', flexDirection: 'column', minWidth: '150px', color: schedule?.active_cron && schedule?.creator_fund == 100 ? '#000' : '#cde', height: '113px', border: '1px solid #eee', borderRadius: '16px', justifyContent: 'center', backgroundColor: schedule?.active_cron && schedule?.creator_fund == 100 ? '#eeeeeebb' : '#111122bb'}}>
            ...
            <div style={{display: 'flex', flexDirection: 'row', justifyContent: "center", alignItems: 'center', gap: '0.5rem'}}>
              {/* <Impact size={15} color={schedule?.active_cron && schedule?.creator_fund == 100 ? '#147' : '#5af'} /> */}
              <div>Creator</div>
              <div style={{fontSize: '14px', fontWeight: '700'}}>100%</div>
            </div>
            <div style={{display: 'flex', flexDirection: 'row', justifyContent: "center", alignItems: 'center', gap: '0.5rem'}}>
              {/* <IoBuild size={15} color={schedule?.active_cron && schedule?.creator_fund == 100 ? '#147' : '#5af'} /> */}
              <div>Dev</div>
              <div style={{fontSize: '14px', fontWeight: '700'}}>0%</div>
            </div>
            <div style={{display: 'flex', flexDirection: 'row', justifyContent: "center", alignItems: 'center', gap: '0.5rem'}}>
              {/* <IoIosRocket size={15} color={schedule?.active_cron && schedule?.creator_fund == 100 ? '#147' : '#5af'} /> */}
              <div>Growth</div>
              <div style={{fontSize: '14px', fontWeight: '700'}}>0%</div>
            </div>
            {/* <div style={{display: 'flex', flexDirection: 'row', margin: '5px 0 0 0', color: schedule?.active_cron && schedule?.creator_fund == 100 ? '#111' : "#9df", fontSize: '11px'}}>
          ...
          <div className={`btn-select ${schedule?.active_cron && schedule?.creator_fund == 80 ? 'cast-act-lt btn-brd-lt' : 'blu-drk btn-brd'}`} style={{display: 'flex', flexDirection: 'column', minWidth: '150px', color: schedule?.active_cron && schedule?.creator_fund == 80 ? '#000' : '#cde', height: '113px', border: '1px solid #eee', borderRadius: '16px', justifyContent: 'center', backgroundColor: schedule?.active_cron && schedule?.creator_fund == 80 ? '#eeeeeebb' : '#111122bb'}}>
            ...
            <div style={{display: 'flex', flexDirection: 'row', justifyContent: "center", alignItems: 'center', gap: '0.75rem'}}>
              {/* <Impact size={15} color={schedule?.active_cron && schedule?.creator_fund == 80 ? '#147' : '#5af'} /> */}
              <div>Creator</div>
              <div style={{fontSize: '14px', fontWeight: '700'}}>80%</div>
            </div>
            <div style={{display: 'flex', flexDirection: 'row', justifyContent: "center", alignItems: 'center', gap: '0.75rem'}}>
              {/* <IoBuild size={15} color={schedule?.active_cron && schedule?.creator_fund == 80 ? '#147' : '#5af'} /> */}
              <div>Dev</div>
              <div style={{fontSize: '14px', fontWeight: '700'}}>10%</div>
            </div>
            <div style={{display: 'flex', flexDirection: 'row', justifyContent: "center", alignItems: 'center', gap: '0.75rem'}}>
              {/* <IoIosRocket size={15} color={schedule?.active_cron && schedule?.creator_fund == 80 ? '#147' : '#5af'} /> */}
              <div>Growth</div>
              <div style={{fontSize: '14px', fontWeight: '700'}}>10%</div>
            </div>
            {/* <div style={{display: 'flex', flexDirection: 'row', margin: '5px 0 0 0', color: schedule?.active_cron && schedule?.creator_fund == 80 ? '#111' : "#9df", fontSize: '11px'}}>
          ...
          <div className={`btn-select ${schedule?.active_cron && schedule?.creator_fund == 60 ? 'cast-act-lt btn-brd-lt' : 'blu-drk btn-brd'}`} style={{display: 'flex', flexDirection: 'column', minWidth: '150px', color: schedule?.active_cron && schedule?.creator_fund == 60 ? '#000' : '#cde', height: '113px', border: '1px solid #eee', borderRadius: '16px', justifyContent: 'center', backgroundColor: schedule?.active_cron && schedule?.creator_fund == 60 ? '#eeeeeebb' : '#111122bb'}}>
            ...
            <div style={{display: 'flex', flexDirection: 'row', justifyContent: "center", alignItems: 'center', gap: '0.75rem'}}>
              {/* <Impact size={15} color={schedule?.active_cron && schedule?.creator_fund == 60 ? '#147' : '#5af'} /> */}
              <div>Creator</div>
              <div style={{fontSize: '14px', fontWeight: '700'}}>60%</div>
            </div>
            <div style={{display: 'flex', flexDirection: 'row', justifyContent: "center", alignItems: 'center', gap: '0.75rem'}}>
              {/* <IoBuild size={15} color={schedule?.active_cron && schedule?.creator_fund == 60 ? '#147' : '#5af'} /> */}
              <div>Dev</div>
              <div style={{fontSize: '14px', fontWeight: '700'}}>20%</div>
            </div>
            <div style={{display: 'flex', flexDirection: 'row', justifyContent: "center", alignItems: 'center', gap: '0.75rem'}}>
              {/* <IoIosRocket size={15} color={schedule?.active_cron && schedule?.creator_fund == 60 ? '#147' : '#5af'} /> */}
              <div>Growth</div>
              <div style={{fontSize: '14px', fontWeight: '700'}}>20%</div>
            </div>
            {/* <div style={{display: 'flex', flexDirection: 'row', margin: '5px 0 0 0', color: schedule?.active_cron && schedule?.creator_fund == 60 ? '#111' : "#9df", fontSize: '11px'}}>
