...
export default async function handler(req, res) {
  const { tip, time, curators, channels, tags, shuffle, referrer, eco, ecosystem, refresh, retry, start } = req.query;
  ...
  if (req.method !== 'POST' || !ecosystem || !eco) {
    ...
    const exploreLink = `${baseURL}/~/ecosystems/${ecosystem}?${qs.stringify({    
      time: time, 
      curators: curators,
      shuffle: false,
      referrer: referrer,
    ...
    const retryPost = `${baseURL}/api/frames/tips?${qs.stringify({    
      tip: 0,
      time: time, 
      curators: curators,
      shuffle: true,
    ...
    let shareUrl = `https://impact.abundance.id/~/ecosystems/${ecosystem}/tip-share?${qs.stringify({    
      tip: 0,
      time: time, 
      curators: curators,
      shuffle: true,
