...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    let autoTipImg = `${baseURL}/api/frames/tip/auto-tipping?${qs.stringify({ status: 'self', curators: [curatorFid], points })}`
    ...
    if (!encryptedUuid) {
      ...
      async function setSchedule(fid, code, points, ecoName, encryptedUuid) {
        let schedule = null
        try {
          await connectToDatabase();
      ...
      const schedule = await setSchedule(curatorFid, code, points, ecoName, encryptedUuid)
      ...
      if (schedule?.active_cron) {
        ...
        if (schedule?.search_curators?.length > 0) {
          ...
          autoTipImg = `${baseURL}/api/frames/tip/auto-tipping?${qs.stringify({ status: 'self', curators: [curatorFid], points })}`
        }
