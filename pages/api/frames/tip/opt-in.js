...
export default async function handler(req, res) {
  ...
  if (req.method !== 'POST' || !ecosystem || !eco) {
    ...
    try {
      ...
      async function deleteOptOut(fid) {
        try {
          const optOutDoc = await OptOut.findOneAndDelete({ fid: fid });
          if (optOutDoc) {
            console.log(`OptOut document deleted for fid: ${fid}`);
            return true;
          } else {
            console.log(`No OptOut document found for fid: ${fid}`);
            return false;
          }
        } catch (error) {
          console.error("Error deleting OptOut document:", error);
          return false;
        }
      }
      
      const optedIn = await deleteOptOut(curatorFid)
