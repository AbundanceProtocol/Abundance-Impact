...
export default async function handler(req, res) {
  ...
  if (req.method !== 'POST') {
    ...
    if (needLogin) {
      ...
      <meta property="fc:frame:button:2:target" content='https://impact.abundance.id/api/frames/console/test?${qs.stringify({ iB, qB, qT, author, iA, qA, ecosystem, login, pt, cu, impact, quality, cI, hash })}' />`
    } else if (parseInt(cI) == 0 && parseInt(impact) == 0 && parseInt(quality) == 0) {
      console.log('2')

      button1 = `<meta property="fc:frame:button:1" content='+1 ${pt}' />
      <meta property="fc:frame:button:1:action" content="post" />
      <meta property="fc:frame:button:1:target" content='https://impact.abundance.id/api/frames/console/test?${qs.stringify({ iB, qB, qT, author, iA, qA, ecosystem, login, pt, cu, impact, quality, cI, hash })}' />`
