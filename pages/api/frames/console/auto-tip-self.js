...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    let autoTipImg = `${baseURL}/api/frames/console/auto-tipping?${qs.stringify({ status: 'self', curators: [curatorFid] })}`
    ...
    if (!encryptedUuid) {
      ...
      if (schedule?.active_cron) {
        ...
        if (schedule?.search_curators?.length > 0) {
          ...
          autoTipImg = `${baseURL}/api/frames/console/auto-tipping?${qs.stringify({ status: 'self', curators: [curatorFid] })}`
        }
