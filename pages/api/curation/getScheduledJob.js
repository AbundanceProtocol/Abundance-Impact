...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET' && fid && encryptedUuid) {
    ...
    const { shuffle, timeRange, tags, channels, curators, percent } = await getSchedule(encryptedUuid)
    ...
    if (percent) {
      ...
      async function determineDistribution(ulfilteredCasts, tip, fid) {
        ...
        if (casts && tip) {
          ...
          const confirmCasts = await postMultipleTips(uuid, fid, combinedLists)
          if (confirmCasts) {
            return true
          } else {
            return null
    ...
    res.status(405).json({ error: 'Method Not Allowed', message: 'This route only accepts POST requests.' });
  }
}