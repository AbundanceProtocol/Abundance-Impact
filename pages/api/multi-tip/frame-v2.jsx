...
export default async function handler(req, res) {
  ...
  try {
    ...
    async function getCircle(id) {
      try {
        ...
        if (circle && !circle?.image) {
          return {showcase: circle.showcase, curator: circle.curator || [], timeframe: circle.time || '', channels: circle.channels || [], image: null}
        } else if (circle && circle?.image) {
          return {showcase: circle.showcase, curator: circle.curator || [], timeframe: circle.time || '', channels: circle.channels || [], image: circle.image}
        } else {
          return {showcase: [], curator: [], timeframe: '', channels: [], image: null}
        }
      } catch (error) {
    ...
    let {showcase, curator, timeframe, channels, image} = await getCircle(id);
