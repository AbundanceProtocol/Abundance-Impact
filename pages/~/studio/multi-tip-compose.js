...
export default function MultiTip() {
  ...
  async function setupEcosystem(target) {
    ...
    let shareUrl = `https://impact.abundance.id/~/curator/${frameCurators?.url ? frameCurators?.url[0] : frameFid}`
    ...
    if (frameCurators?.url && Number(frameCurators?.url[0]) !== Number(frameFid)) {
