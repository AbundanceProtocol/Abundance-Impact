...
export default function Tips({time, curators, channels, tags, eco, ecosystem, fids, text, username, id, tipperFid}) {
  ...
  useEffect(() => {
    ...
    updatedFrameData.image = `${baseURL}/api/multi-tip/frame-v2?${qs.stringify(
      { id }
    )}`;
    ...
    >
      {queryData && (
        <Head>
          ...
          <meta property="og:title" content="Curation" />
          <meta
            property="og:image"
            content={`${baseURL}/api/multi-tip/frame-v2?${qs.stringify({
              id,
            })}`}
          />
          ...
          <meta
            ...
            content={`https://impact.abundance.id/?eco=$${eco}`}
          />
