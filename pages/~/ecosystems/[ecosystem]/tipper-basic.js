...
export default function Tips({time, curators, channels, tags, eco, ecosystem, hash}) {
  ...
  const initFrame = 
    {
      ...
      buttons: [
        ...
        },
        {
          ...
          title: "Refresh",
          action_type: "post",
          target: `${baseURL}/api/frames/console/tip-refresh?tip=0`
        }
      ],
      input: {
  ...
  useEffect(() => {
    ...
    updatedFrameData.buttons[1].target = `${baseURL}/~/curator/${curators}?${qs.stringify({ points: '$' + eco })}`
  ...
  return (
    ...
    {queryData && (
      <Head>
        ...
        <meta property="fc:frame:button:1:target" content={`${baseURL}/api/frames/console/tip-tip?${qs.stringify({ time, curators, eco, ecosystem, hash })}`} />

        <meta property="fc:frame:button:2" content={`Explore curation`} />
        <meta property="fc:frame:button:2:action" content="link" />

        <meta property="fc:frame:button:2:target" content={`${baseURL}/~/curator/${curators}?${qs.stringify({ points: '$' + eco })}`} />
