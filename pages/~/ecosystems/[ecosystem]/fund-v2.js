...
export default function Tips({ecosystem, referrer, id}) {
  ...
  const shareText = `Auto-fund Open Source builders on farcasters with your daily (remaining) $degen & $ham thru /impact's @impactfund 👇`

  let shareUrl = `https://impact.abundance.id/~/ecosystems/${ecosystem || 'abundance'}/fund-v2?${qs.stringify({referrer: referrer || null})}`
  ...
  const initFrame = 
    {
      version: "vNext",
      title: "Multi-Tip",
      image: `${baseURL}/api/frames/fund/frame-v2?${qs.stringify({ id })}`,
  ...
  useEffect(() => {
    ...
    updatedFrameData.image = `${baseURL}/api/frames/fund/frame-v2?${qs.stringify({ id })}`
  ...
  return (
    ...
    {queryData && (
      <Head>
        ...
        <meta property='og:image' content={`${baseURL}/api/frames/fund/frame-v2?${qs.stringify({ id })}`} />
        <meta property="fc:frame" content="vNext" />
        <meta property="fc:frame:image" content={`${baseURL}/api/frames/fund/frame-v2?${qs.stringify({ id })}`} />
