...
const Curators = ({curator, index}) => {
  ...
  return (
    <div className='flex-row' style={{alignItems: 'center', justifyContent: 'flex-start'}}>
      ...
      <div style={{fontSize: '15px', fontWeight: '500', padding: '6px 10px', textWrap: 'nowrap'}}><Link href={`/~/ecosystems/${ecoData?.ecosystem_handle}/curators/${curator?.username}` }>@{curator?.author_name}</Link></div>
    </div>
  )
}
