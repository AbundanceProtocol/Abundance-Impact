...
const Curators = ({curator, index}) => {
  ...
  return (
    <div className='flex-row' style={{alignItems: 'center', justifyContent: 'flex-start'}}>
      ...
      <div style={{fontSize: '15px', fontWeight: '500', width: '15px', padding: '6px 10px', textWrap: 'nowrap'}}><Link href={'/~/curator/' + curator?.fid || 9326 + '?points=' + points }>@{curator?.author_name}</Link></div>
    </div>
  )
}
