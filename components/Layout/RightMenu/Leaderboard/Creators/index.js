...
const Creators = ({creator, index}) => {
  ...
  const { LoginPopup, ecoData } = useContext(AccountContext)
  ...
  return (
    <div className='flex-row' style={{alignItems: 'center', justifyContent: 'flex-start'}}>
      ...
      <div style={{fontSize: '15px', fontWeight: '500', width: '15px', padding: '6px 10px', textWrap: 'nowrap'}}><Link className="fc-lnk" href={`/~/ecosystems/${ecoData?.ecosystem_handle}/creators/${creator?.author_name}`} onClick={(event) => {if (!store.isAuth) {
        console.log('cr1')
