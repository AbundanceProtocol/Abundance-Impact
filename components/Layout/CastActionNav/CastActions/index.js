...
const CastActions = () => {
  ...
  return (
    <div className='flex-col' style={{gap: '0.5rem', margin: '8px'}}>
      {ecoData?.curate_user !== true && (<a className="" title={`${points} Dashboard`} href={`https://warpcast.com/~/add-cast-action?name=%24${points?.substring(1)}+Console&icon=star&actionType=post&po${points?.substring(1)}&description=Curate+Casts+with+the+Impact+App`} target="_blank" rel="noopener noreferrer">
