...
const CastActions = () => {
  ...
  return (
    <div className='flex-col' style={{gap: '0.5rem', margin: '8px'}}>
      <a className="" title={`+1 ${points}`} href={`https://warpcast.com/~/add-cast-action?name=%2B1+%24${points?.substring(1)}&icon=star&actionType=post&postUrl=https%3A%2Fimpact.abundance.id%2Fapi%2Faction%2Fimpact1%3Fpoints=${points?.substring(1)}&description=Curate+Casts+with+the+Impact+App`} target="_blank" rel="noopener noreferrer">
      ...
      <a className="" title={`+5 ${points}`} href={`https://warpcast.com/~/add-cast-action?name=%2B5+%24${points?.substring(1)}&icon=star&actionType=post&postUrl=https%3A%2Fimpact.abundance.id%2Fapi%2Faction%2Fimpact5%3Fpoints=${points?.substring(1)}&description=Curate+Casts+with+the+Impact+App`} target="_blank" rel="noopener noreferrer">
        <div className='flex-row cast-act' style={{borderRadius: '8px', padding: '8px 4px', alignItems: 'center', justifyContent: 'center', gap: '0.25rem'}}>
          <FaRegStar size={20} />
